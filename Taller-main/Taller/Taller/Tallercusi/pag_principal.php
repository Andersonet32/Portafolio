...
<body>
    ...
    <section class="table-section">
        ...
        <table>
            ...
            <tbody>
                <?php
                $sql = "SELECT Clientes.nombre AS cliente, 
                Vehiculos.marca, Vehiculos.modelo AS Vehiculos, 
                TiposServicio.nombre_servicio AS Tiposservicio,
                Reservas.fecha_reserva, Reservas.estado, Reservas.observaciones from Reservas
                ...
                if ($result = $conn->query($sql)) {
                    if ($result->num_rows > 0) {
                        echo "Se encontraron " . $result->num_rows . " reservas.";
                        while($row = $result->fetch_assoc()) {
                            echo "<tr>
                                    <td>{$row['cliente']}</td>
                                    <td>{$row['marca']} {$row['modelo']}</td>
                                    <td>{$row['fecha_reserva']}</td>
                                    <td>{$row['TiposServicio']}</td>
                                    <td>{$row['estado']}</td>
        ...
        <section class="button-section">
            <button onclick="location.href='editar_reserva.php'">Editar Reserva</button>
        </section>
        ...
        <section class="button-section">
            <button onclick="location.href='editar_inventario.php'">Editar Inventario</button>
        </section>
    </section>
